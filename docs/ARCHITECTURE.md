...
> to the existing LLM `guessChances` path when the Oracul is unavailable).
...
`PoolAggregateResponse`: which Oracul build, which LLM models, and which method (`live` |
...
> **Why Oracul probabilities stay in ~20–80% (by design, not a clamp).** There is
...
> clamped to `[0, 1]` and so was the only path by which the Oracul could publish
...
> its members. The tradeoff is the flip side — the Oracul **also cannot express
...
> actually justifies (this was the root cause of a real Oracul/market divergence on
