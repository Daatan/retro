...
class ApiSettings(BaseSettings):
    ...
    # umbrella #779) ─────────────────────────────────────────────────────
    # Balances pool mass across outlet lineage strata (country+language in
    # v1 — `data/source_strata.json`, generated by
    # `api/scripts/generate_source_strata_782.py` from the already-curated
    # `data/sources/*.json` catalog) instead of across individual outlets:
    # three outlets from one stratum should not out-vote a stratum
    # represented by a single outlet. See
    # `aggregation.source_lineage_stratum_factors` for the mechanism and the
    # deliberate departure from the issue's literal (mass-normalizing-to-1.0)
    # spec in favor of `cap_source_mass`'s grand-total-preserving convention.
    #
    # `min_stratum_share` is the issue's own "one number must be chosen"
    # threshold — a stratum below this share of total pool weight is folded
    # into a shared "other" bucket rather than getting its own vote, so one
    # fringe/unmapped outlet cannot claim a full stratum's worth of
    # influence. The issue's text is explicit that this must come from an
    # offline measurement (`api/scripts/measure_source_strata_782.py`), not
    # a backtest — 0.0 here is a placeholder (accepts every stratum,
    # including singletons) rather than a proposed value, since the flag
    # below ships OFF and nothing depends on this number until Mark reviews
    # the measurement and turns it on.
    #
    # Both ship OFF/inert: `source_lineage_stratum_balance=False` is the
    # shipped default (mirrors `harmonic_source_discount`) — unlike Rule 1
    # (`event_key_dependence_collapse`), this rule needs the threshold above
    # tuned from real prod data before it can justify shipping ON. Flipping
    # it on changes already-published pool weights, so a republish sweep
    # (this repo's CLAUDE.md) is required before it counts as live for
    # existing forecasts.
