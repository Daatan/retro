...
def test_fetch_url_extracts_via_safe_get():
    ...
    with (
        patch("forecast_api.article_fetch.safe_get", return_value=_resp()) as sg,
...
def test_fetch_url_rejects_internal_before_fetch():
    ...
    with patch("forecast_api.article_fetch.safe_get") as sg:
...
def test_fetch_url_blocks_when_safe_get_raises():
    ...
    with patch("forecast_api.article_fetch.safe_get", side_effect=UnsafeURLError("blocked")):
        r = client.post("/fetch-url", json={"url": "http://93.184.216.34/a"})
    assert r.status_code == 422
