...
def test_count_tournament_questions_uses_count_field_when_present():
    requests: list[httpx.Request] = []
    page = httpx.Response(200, json={"results": [{"id": 1}, {"id": 2}], "next": "http://x/?offset=2", "count": 337})
    transport = httpx.MockTransport(_handler(requests, {("GET", "/api/posts/"): page}))

    with MetaculusClient("tok", transport=transport) as client:
        total = client.count_tournament_questions("fall-futureeval-2026")
...
def test_count_tournament_questions_passes_forecast_type_filter():
    requests: list[httpx.Request] = []
    page = httpx.Response(200, json={"results": [], "next": None, "count": 169})
    transport = httpx.MockTransport(_handler(requests, {("GET", "/api/posts/"): page}))

    with MetaculusClient("tok", transport=transport) as client:
        total = client.count_tournament_questions("fall-futureeval-2026", forecast_type="binary")
...
def test_count_tournament_questions_falls_back_to_walking_pages_without_count():
    requests: list[httpx.Request] = []
    responses = iter(
        [
            httpx.Response(200, json={"results": [{"id": 1}], "next": "http://x/?offset=1"}),
    ...
    transport = httpx.MockTransport(handle)
    with MetaculusClient("tok", transport=transport) as client:
        total = client.count_tournament_questions("bot-testing-area")

    assert total == 2
    assert len(requests) == 2


def test_count_tournament_questions_empty_tournament_returns_zero():
    requests: list[httpx.Request] = []
    page = httpx.Response(200, json={"results": [], "next": None})
    transport = httpx.MockTransport(_handler(requests, {("GET", "/api/posts/"): page}))

    with MetaculusClient("tok", transport=transport) as client:
        assert client.count_tournament_questions("bot-testing-area") == 0
