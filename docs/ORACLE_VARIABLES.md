...
| `quantity` | `{value, unit, comparator: =\|<\|<=\|>\|>=\|between, value_hi, as_of}`, optional | the **number** the quote reports about the event, with the relation the ARTICLE asserts about it | **EXPERIMENTAL, shadow** (Oracle 1.5 Phase 1, retro#683 — unparked from retro#664's P2): populated, persisted, read by nothing. The model is asked for the FIGURE and never for the verdict: whether it clears the question's bar is arithmetic and is done in code (`tm.threshold_compare`), because PR#671 measured Nova Lite returning stance +0.00 on every between-bounds numeric case with three prompt sections already telling it to compare. That is why #664's P2 resolved as *the field, not another prompt fix*. Kept strictly apart from `quantitative_estimate`, which retro#362 narrowed to a cited PROBABILITY of the event — a share, count, rate or tonnage lands here and leaves that field null. Per-claim, **no article-level rollup**: an article can report several figures and reducing them would have to pick one with nothing asking it to. **Rater-dependent and the ONE caveat every consumer must carry:** measured exact `(value, unit, comparator)` on the numeric corpus, Haiku 4.5 scores 100% and Nova Lite 80%, and *every* Nova Lite error is `comparator` — a movement verb written as a bound ("accelerated to 4.1 percent" → `> 4.1`). Two prompt edits aimed at it made it worse and were reverted. Phase 2 must therefore gate on the **rater**, not on the field being filled |
| `tone` | `approve` \| `neutral` \| `alarm` | how the quote **feels** about what it reports — its own register, not its direction | **EXPERIMENTAL, shadow** (Oracle 1.5 Phase 1, retro#684 — unparked from retro#673): populated, persisted, read by nothing. The leak retro#326 and retro#657 keep patching by prompt is a *projection* problem — an evaluation being read as a direction — and no amount of "alarming tone is not stance direction" prose removes an axis the schema has no place to put. Eliciting it separately is what lets Phase 3 S4 project it out at rating time instead. Never moves `stance`, `claim_strength` or anything else: `alarm` at stance +1.0 is the combination the field exists for. Always answered, `neutral` included, so on a v10 row a null means the model did not answer rather than that the quote was even-handed — that distinction is the fill measurement. Kill criterion: >90% `neutral` on live rows |
...
| `grounds` | `{kind: observed_milestone\|official_statement\|market_or_poll_figure\|analyst_inference\|precedent_or_base_rate\|authors_judgement, basis?}`, optional | what the quote's position **rests on** — the reason, not the direction | **EXPERIMENTAL, shadow** (Oracle 1.5 Phase 1, retro#763 — unparked from retro#673 §1): populated, persisted, read by nothing. Three outlets repeating one ministry statement agree for ONE reason; three citing a milestone, a poll and a precedent agree for three — the pool's `n_eff` counts articles where it means to count reasons, and `kind` is the closed pick that count is taken over, `basis` the phrase that lets two `official_statement` rows be recognised as the *same* statement (the F2/F16 same-grounds-same-stance dedup). **Not an `evidence_class` extension** — that enum stays flat, permanently: class is the *route* the information took (reported / cited / opinion), grounds is *what was seen* at the far end, and the two vary independently (a columnist reasoning from a poll is `opinion` by class and `market_or_poll_figure` by grounds). Per claim on `ClaimDetail`, rolled up to `SourceSignal` as the **dominant** claim's (the claim `facet`, `verified` and `voice` ride from), because the n_eff-over-reasons counts at the grain of the source. Kill criterion per #673's own caveat: >90% of claims in one `kind` on the live rater means the pick carries no information — per-rater distributions in `PROMPT_VERSIONS.md` v12. |
...
| `stance_vs_quantity` | the claim's own number and its stance sign disagree about a threshold question | **inert** — the claim side landed with retro#683 (08-30); the question side has not, see below |
...
**`stance_vs_quantity` is still inert, but for one reason now rather than two.** It needs
a claim-side number (`quantity`, retro#683) and a question-side threshold (Phase 2's
`question_quantity`). **The claim side landed on 2026-08-30** — `quantity` is elicited,
projected and persisted, and the rule reads it via `getattr` exactly as designed, so no
edit was needed here for it. The question side does not exist, so the rule still yields no
flag on every row; it self-activates the moment Phase 2 lands `question_quantity`, which is
also when the comparison itself moves to `tm.threshold_compare` (retro#683 wired the
code-side comparison; retro#748 widened the `is_threshold_shaped` detector that routes to
it). One caveat carried from #683's measurement and not to be lost here: on **Nova Lite**
`quantity.comparator` is wrong 18% of the time — a movement verb written as a bound — so
this rule must gate on the RATER, not merely on the field being non-null, or it will flag
on the extractor's own error rather than on the article's. `ClaimDetail.quantitative_estimate`
