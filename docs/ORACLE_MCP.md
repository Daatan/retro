...
| `search_news(query, limit?, date_from?, date_to?, min_results?)` | `read` | 60/min | News search via the provider chain. `min_results` > 0: when news-indexer serves fewer hits, the paid providers top the set up (dedup by URL, up to `limit`; `provider` reads `news_indexer+<paid>`) — retro#822. The paid pass is capped at `SEARCH_TOPUP_BUDGET_S` (15 s): on expiry the index hits come back alone and `provider_chain` ends in `topup_timeout` (retro#824) |
