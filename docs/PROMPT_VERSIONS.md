...
| extractor | v13 | `4e3e72bb90e91646` | 2026-09-01 | retro#770 | **A behavioural rule, not a new field** — nothing added to `ExtractionOutput`, so schema, `PROVENANCE_SCHEMA_VERSION`, `ClaimDetail` and the `forecaster.py` projection are all untouched, and there is no daatan-side render question to open. New `## An office title is how an article NAMES someone — it is not a report on their tenure` section, placed after the capability/intent cap pair and before `## STANCE`: an office/rank/title used to IDENTIFY a person is a naming convention, so on a question about whether they hold that office AT A FUTURE DATE it is a precondition (`\|stance\| <= 0.2`, `claim_strength <= 0.3`, `is_occurrence false`, never settled), and a fragment carrying no proposition (a broadcast billing, a caption, a colon lead-in) extracts NOTHING. Reuses the proven capability/intent cap shape rather than a new mechanism. Five worked examples, two of which are POSITIVE controls (no-confidence survival, resignation) so the rule cannot be read as "ignore incumbent coverage" — the retro#720 failure mode. Prompt 70,926 → 73,382 chars (+2,456, +3.5%); schema unchanged at 29,783. **The defect is Haiku-only and the anonymised A/B could not see it.** Cases written with a placeholder "Leader L" reproduce nothing on either rater — the bug needs the model's own world knowledge that Netanyahu IS the incumbent, so the faithful cases carry the real entity, the real question and the real article text (`scripts/ab_cases/honorific_real_rows.json`; the anonymised set is kept as `honorific_office_title.json`, useful only as controls). **Measured, 5 corpus files, 3 runs/case, both raters. Haiku 4.5: `gate_exit_code` 0 — 24 pass, 2 unchanged (`seat-projection-facet-not-announcement`, `decider-announcement-facet-control`, both already failing on baseline), zero regressions; `token_usage` 2,035,845 → 2,080,789 (+2.2%).** On the real defect rows, 3/3 runs each: the Magal billing (`t.me/yinonews/61432`, "הערב בפטריוטים: / ראש הממשלה / בנימין נתניהו") goes **+1.0 / settled=true / is_occurrence=true → no extraction**, and the English passing-title form goes **+0.9 / is_occurrence=true → +0.2, claim_strength 0.3, is_occurrence false** — the cap as written. The tenure control keeps its full negative signal (−0.6/−0.5/−0.7 → −0.6/−0.7/−0.7), so this is not blunt suppression. **Nova Lite: `gate_exit_code` 1 at 3 runs — one regression, `bracket-final-stage-cleared` losing `stance_band`, against four improvements** (`bracket-runoff-first-round-lead`, `threshold-at-or-below-satisfied`, and both `stance-tone-conflation` cases); `token_usage` 1,875,719 → 1,917,963 (+2.3%). **That verdict is sample-size dependent and does not survive re-measurement**: at 15 runs per arm the case is satisfied on BOTH (the gate would pass), with the moderate-band rate 7/15 baseline vs 2/15 patched — a real narrowing, not a lost capability. Nova never had the honorific defect to begin with (0/15 runs ≥0.95 on the Magal case at baseline) and shows 1/13 patched — one occurrence, not significant. **Consequence: gate on Haiku.** daatan's evidence pool only ever calls the live API, which is Haiku (the defect row's own provenance is `bedrock/us.anthropic.claude-haiku-4-5`, prompt v10), so every pool row this fixes comes from the rater that passes cleanly; Nova's exposure is the batch/atlas lane, where the measured effect is a band narrowing on one bracket case. **Not fixed here, deliberately:** retro#770's class 2 (sign inversion on "X takes office") is the retro#657 negation shape that has failed three times on Nova — it needs a different mechanism, not a fourth clause — and classes 3 and 4 (`settled=true` on non-events, re-scoring instability >1.0 on identical input) stay open. |
