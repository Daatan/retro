...
### Removed
- Extractor prompt **v14** (retro#774): the `grounds` elicitation added by v12d is **withdrawn**. The field leaves `PredictionExtraction`, so it leaves the JSON schema instructor serialises into every call and the model stops being asked; the `Grounds` class, the `ClaimDetail`/`SourceSignal` wire fields and the rows already written keep working, so `provenance.schema_version` stays **1.6**. Withdrawn because asking for it cost `evidence_class`, which is load-bearing: on prod pool rows carrying an `extractor_prompt_version`, unclassified ran 35/10,945 (0.32%) before `grounds` vs 10/235 (4.26%) on v12+v13 — ~13x, and each unclassified claim is capped at 0.25, 4x below `reported_fact`. All 18 logged leaks were `grounds.kind` members against zero in the preceding 4.5 months. This is also the acceptance A/B `PROMPT_VERSIONS.md` recorded v12d as never having had, and the criterion is not met. Measured, 30 cases x 3 runs: **Haiku 4.5 gate 0 — zero regressions, 2 improvements, `evidence_class` fill 98.4% -> 100%, `fact_signal` unchanged at 73.0%, `token_usage` -5.0%**; **Nova Lite gate 1 — 3 regressions surviving 15 runs, root-caused to `fact_signal` fill falling 53.9% -> 37.3%** (retro#680's failure mode reached from the schema tail rather than the middle). Gated on Haiku, the only rater daatan's evidence pool ever calls; the Nova batch-lane cost is filed separately. A compensating clause was tried, made both raters worse, and was reverted rather than iterated on

### Fixed
- A/B harness (retro#757): a `volatile: true` corpus tag forces a case to run at `VOLATILE_MIN_RUNS` (15) regardless of `--runs-per-case`, and `compare` flags any regression measured at fewer than `CONFIDENT_MIN_RUNS` (15) runs as `LOW CONFIDENCE` — informational only, the gate's exit code is unchanged. Tagged the four cases retro#697 found unstable at the 5-run default (`threshold-at-or-below-satisfied`, `threshold-tone-negative-number-satisfies`, `decider-denial-regression-sentinel`, `stance-tone-conflation-hazard-persists-control`)
- A/B harness (retro#775): `unmet_facets()` now accepts `expect: {"no_predictions": true}` as a strict, all-runs-must-be-empty facet, so a case where the correct extraction is *nothing* can pass instead of always scoring as a miss
- `polymarket_harvest.py` (retro#764): switched pagination from Gamma's offset-based `/markets` endpoint, which hard-caps around ~2,100 markets, to the CLOB `/markets` endpoint's cursor pagination; a non-200 mid-corpus is now logged and treated as a transient failure instead of silently looking like end-of-pagination
...
### Added
- `audit_scheduled_deadline_unconfirmed` (retro#590, proposal 3 of retro#575): log-only shadow signal — does a scheduled/threshold claim's deadline pass with the covering article published after it, and no `event_date` ever confirming the outcome? Neither `aggregation.settlement_direction_allowed`/`settlement_vote_validity` (compare the deadline at pool-aggregation time, only once a settlement vote exists) nor daatan's `temporal-clock.ts` (a post-pool wrapper) previously represented this — an article whose coverage moves on without confirming the outcome was invisible. Live path only (`forecaster.py`'s `_process_article`); the batch/atlas pipeline is out of scope for the same reason `runner.py` already documents for `enforce_deadline_arithmetic` — no per-article `claim_deadline`/`claim_archetype` there. Fail-open: never mutates `stance`/`claim_strength`/`settled`. Same shadow-first rollout pattern as `audit_named_entity_dyad_mismatch` (retro#545 slice ii) — trigger rate/value unmeasured
- Leave-one-source-out sensitivity reporting, `aggregation.leave_one_source_out_sensitivity` (retro#783, source-dependence Rule 4, umbrella #779): for each distinct `source_id` carrying positive weight, re-pool with that outlet's rows zeroed and diff the resulting mean against the full pool's — exposes `max_swing`/`dominant_source` on `PoolAggregateResult`, logged on both the live and recompute paths as `event=source_sensitivity`. Reporting only, always on (no feature flag, no threshold to choose) — never reads back into `mean`/`std`/`ci_low`/`ci_high`, so no republish-sweep obligation. The umbrella's own stated first step ("Then #783 (diagnostic) -> #780 -> #781 -> #782") and the acceptance test for rules 1-3: the distribution of `max_swing` across live pools should fall once they land. Not yet wired into `ForecastResponse`/`PoolAggregateResponse` — the diagnostics block the issue asks for (daatan#1644 §2.3) is itself an unbuilt stub; ships log-only for now, the same shape `hazard_shadow_mean` (retro#356) already ships in
- Event-key echo collapse, `aggregation.event_key_collapse_factors` (retro#780, source-dependence Rule 1, umbrella #779): rows sharing an `event_key` (retro#682's paraphrase-invariant `(actor, target, day)` triple) are scaled to one row's worth of weight — every member of the group is multiplied by `max(group weights) / sum(group weights)`, which leaves the group's weighted-mean stance unchanged (uniform scaling cancels out of a weighted average) while shrinking its total pool contribution from the sum of all echoing rows to its single loudest member's. **Event-key-only, no attribution/text-Jaccard corroboration required** — `scripts/measure_dependence_key_780.py`'s Step 0 measurement found the combined spec as literally written barely fires (0.2% row collapse, 2.38pp max needle move, 0/120 pools >= 5pp, because `voice.attributed_to` fills only 2.7% of dominant claims today) versus event-key-only at 2.3% collapse, up to 11.51pp, 1/120 pools >= 5pp. Mark's 2026-09-03 decision (issue comment 5524245079) ships the looser, stronger bar now. Applied BEFORE the harmonic per-source discount (retro#781) in `aggregate_pool`, per that discount's own stated dependency on this one running first. **Ships ON** (`settings.event_key_dependence_collapse = True`) — unlike `cluster_downweight_exponent`/`max_source_share`, the umbrella's premise is justification by structural property (duplication invariance) rather than a resolved-forecast backtest, so there is no magnitude to tune and no #403-style gate. **Changes already-published pool weights — the republish sweep this repo's CLAUDE.md requires (`POST /api/admin/forecasts/republish`, dry-run first) has not yet been run as of this PR; that is Mark's call post-merge**
- `metaculus_backtest.py self-resolve` + `score --self-resolved` (retro#737, access-independent fallback for the blocked retro#619): for a Metaculus question with no `resolution` field (the `Daatan-v1` token lacks the Bot Benchmarking Access Tier), search a post-`actual_close_time` window and re-run Oracul in-process to classify the outcome ourselves — decisive only above a `--confidence` threshold (default 0.9), else left unresolved rather than guessed. `score --self-resolved` scores these rows separately from real Metaculus resolutions, under their own "Self-resolved (weaker evidence)" heading, and a real Metaculus resolution always wins when both exist. Per the issue's own honesty constraint, this evidence must never be reported as "Metaculus-validated" and does not satisfy the Oracle 1.5 Gate-0 evidence bar on its own
- Harmonic per-source discount, `aggregation.harmonic_source_discount_factors` (retro#781, source-dependence Rule 2, umbrella #779): within one `source_id` group, rows are ordered by recency and the i-th row's weight is multiplied by `1/i`, so an outlet's total mass grows like the harmonic number `H_k` (log) rather than linearly with row count — 10 rows from one outlet contribute ~2.9 rows' worth, 100 contribute ~5.2. Unlike `max_source_share` (retro#458), the `1/i` shape needs no tuned threshold, so it ships as a plain on/off switch, `settings.harmonic_source_discount`. **Ships off by default** — flipping it on changes already-published pool weights and needs the republish sweep this repo's own hard rules require before it counts as live for existing forecasts
- Source-lineage stratum balance, `aggregation.source_lineage_stratum_factors` (retro#782, source-dependence Rule 3, umbrella #779): balances pool mass across outlet lineage strata (country+language, `data/source_strata.json` generated from the existing `data/sources/*.json` catalog by `api/scripts/generate_source_strata_782.py`) rather than across individual outlets, so several outlets sharing one lineage cannot out-vote a stratum represented by a single outlet. Each stratum's rows are rescaled to carry `1/n_strata_present` of total pool weight, grand-total-preserving per `cap_source_mass`'s convention — a deliberate departure from the issue's literal normalize-to-1.0 spec; strata below `min_stratum_share` merge into one shared "other" bucket; an unmapped outlet is its own singleton stratum, mirroring `cap_source_mass`. `min_stratum_share` is the issue's own "propose from an offline measurement, not a backtest" number — `api/scripts/measure_source_strata_782.py` (Step 0) is included, reusing the real function against a prod pool dump across `[0.0, 0.05, 0.10, 0.15, 0.20, 0.25]`; `0.0` here is a placeholder, not a proposal. **Ships off by default** (`settings.source_lineage_stratum_balance`), mirroring Rule 2 rather than Rule 1's structural-property justification — needs the offline measurement before a threshold and the republish sweep can follow. Incidentally fixes a real `_DOMAIN_MAP` identity bug found while generating the strata map: `haaretz.co.il` (the catalog's actual `haaretz.json` URL) was resolving to the wrong `source_id`, following the same multi-domain-alias precedent already used for `ynet`/`israel_hayom`/`globes`/`walla`; the analogous `n12.co.il`/`mako` gap is left unfixed, no such precedent exists for it
